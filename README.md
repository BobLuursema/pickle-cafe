...
Clone this project: `git clone git@vcs.aws.insim.biz:hyp-klant-worden/pickle-cafe.git`.
Create a folder the project: `mkdir gitlab-test` and `cd gitlab-test`.
Add an npm module of your project `npm init`, answer the questions and then run `npm link ../pickle-cafe` to add Pickle Café to your project, and `npm install cucumber` to add Cucumber.
...
This page needs to be instantiated after `t` has been instantiated. You could write your own After hook for that, so add the following code to `env.js`
...
The last step is to create a `reports` folder, currently PickleCafé does not expose the TestCafé programming interface and the setup requires that folder to exist. The final folder structure should like this:
